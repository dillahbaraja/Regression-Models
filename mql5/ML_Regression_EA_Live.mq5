//+------------------------------------------------------------------+
//|                                    ML_Regression_EA_Live.mq5     |
//|  Live-style chart/news regression trader for MT5 backtesting     |
//+------------------------------------------------------------------+
#property strict
...
input ENUM_LOT_MODE  InpLotMode                = LOT_MODE_DYNAMIC_RISK;
...
input string          InpAssetFolder           = "Regression_Assets\\";
input string          InpNewsCsvFileName       = "NEWS_2024.01.01-2026.06.30_ALL.csv";
input bool            InpOneTradePerEvent      = true;
input bool            InpUseTechnicalFilter    = false;
...
string gDefaultDebugCsvBaseName = "ML_Regression_EA_Live_debug.csv";
string gDebugCsvFileName = "";
string gTransactionCsvFile = "";
string gTradedEventKeys[];
string gInverseEventKeywords = "unemployment;jobless;claimant;inventories";
string gLastEvaluationFailureStatus = "";
string gLastEnsembleTrace = "";
...
bool BuildScaledFeatureVector(const datetime bar_time, vectorf &features, int &max_importance, string &event_name, datetime &selected_event_time, string &selected_event_country, double &selected_event_actual, double &selected_event_forecast, double &selected_event_previous);
...
void ResolveEnsembleWeights(double &weights[]);
int ResolveEnsembleMinimumSupport();
double ResolveEnsembleSupportThresholdFactor();
bool EvaluateEvent(const datetime bar_time, double &prediction, int &importance, string &event_name, datetime &selected_event_time, string &selected_event_country, double &selected_event_actual, double &selected_event_forecast, double &selected_event_previous);
...
string ResolveEventKey(const datetime event_time, const string event_name);
bool HasTradedEventKey(const string event_key);
void RememberTradedEventKey(const string event_key);
bool StringMatchesDelimitedKeyword(const string text, const string keyword_list);
string ResolveGlobalBlockedEventNames();
string ResolveSymbolGeneralBlockedEventNames();
string ResolveSymbolModelBlockedEventNames();
string ResolveGlobalReversedEventNames();
string ResolveSymbolGeneralReversedEventNames();
string ResolveSymbolModelReversedEventNames();
bool EventTypeAllowed(const string event_name);
bool PassTechnicalFilter(const datetime bar_time, const int signal);
bool IsInverseFundamentalEvent(const string event_name);
bool EventSignalShouldBeReversed(const string event_name);
bool PassFundamentalDirectionFilter(const int signal,
                                    const string event_name,
                                    const string event_country,
                                    const double actual,
                                    const double forecast,
                                    const double previous);
string BuildPerRunDebugCsvFileName();
...
bool AppendDebugCsv(const datetime previous_bar_time,
                    ...
                    const double effective_buy_threshold,
                    const double effective_sell_threshold,
                    const int signal,
   ...
   if(IsEnsembleModelType())
     {
      ...
      for(int i = 0; i < ArraySize(ensemble_handles); i++)
        {
         string ensemble_file = gAssetFolder + _Symbol + "_" + ResolveTimeframeString() + "_" + ensemble_model_names[i] + ".onnx";
         if(!InitSingleOnnx(ensemble_file, ensemble_model_names[i], ensemble_handles[i]))
            return false;
        }
      return true;
     }

   string file_name = gAssetFolder + _Symbol + "_" + ResolveTimeframeString() + "_" + ResolveModelString() + ".onnx";
   return InitSingleOnnx(file_name, ResolveModelString(), onnx_handle);
  }
...
bool IsRelevantCountryCode(const string country_code)
  {
   ...
   if(base == "CAD" && code == "CA") return true;
   if(base == "CHF" && code == "CH") return true;
   ...
   if(quote == "CAD" && code == "CA") return true;
   if(quote == "CHF" && code == "CH") return true;
...
bool RefreshNewsEventsIfDue(const bool force_reload=false)
  {
   if(!force_reload && gNewsLoaded && InpNewsReloadSeconds <= 0)
      return true;
...
//+------------------------------------------------------------------+
bool BuildScaledFeatureVector(const datetime bar_time,
                              vectorf &features,
                              int &max_importance,
                              string &event_name,
                              datetime &selected_event_time,
                              string &selected_event_country,
                              double &selected_event_actual,
                              double &selected_event_forecast,
                              double &selected_event_previous)
  {
   event_name = "Economic Event";
   max_importance = 0;
   selected_event_time = 0;
   selected_event_country = "";
   selected_event_actual = 0.0;
   selected_event_forecast = 0.0;
   selected_event_previous = 0.0;
      ...
      if(offset_hours < best_abs_offset || (MathAbs(offset_hours - best_abs_offset) < 0.000001 && gNewsImportances[i] > best_importance))
        {
         ...
         selected_event_country = event_country;
         selected_event_actual = gNewsActuals[i];
         selected_event_forecast = gNewsForecasts[i];
         selected_event_previous = gNewsPreviouss[i];
        }
...
void ResolveEnsembleWeights(double &weights[])
  {
   ArrayResize(weights, ArraySize(ensemble_model_names));
   ...
   if(_Symbol == "AUDUSD")
     {
      weights[0] = 0.10; // xgboost
      weights[1] = 0.25; // lightgbm
      weights[2] = 0.45; // catboost
      weights[3] = 0.20; // sklearn_hgb
      return;
     }
...
int ResolveEnsembleMinimumSupport()
  {
   if(_Symbol == "GBPUSD")
      return 1;
   if(_Symbol == "EURUSD" || _Symbol == "AUDUSD" || _Symbol == "USDJPY")
      return 2;
   return 2;
  }
...
//+------------------------------------------------------------------+
bool EvaluateEvent(const datetime bar_time,
                   double &prediction,
                   int &importance,
                   string &event_name,
                   datetime &selected_event_time,
                   ...
                   double &selected_event_previous)
  {
   gLastEvaluationFailureStatus = "";
   gLastEnsembleTrace = "";

   vectorf features;
   if(!BuildScaledFeatureVector(bar_time,
                                features,
                                importance,
                                event_name,
                                selected_event_time,
                                selected_event_country,
                                selected_event_actual,
                                selected_event_forecast,
                                selected_event_previous))
      return false;

   if(IsEnsembleModelType())
     {
      double weights[];
      ResolveEnsembleWeights(weights);

      double weighted_prediction_sum = 0.0;
      double total_weight = 0.0;
      int valid_models = 0;
      double support_buy_threshold = MathAbs(gActiveThresholdBuy) * ResolveEnsembleSupportThresholdFactor();
      double support_sell_threshold = -MathAbs(gActiveThresholdSell) * ResolveEnsembleSupportThresholdFactor();
      double support_weight = 0.0;
      double opposing_weight = 0.0;
      int support_count = 0;
      int opposing_count = 0;
      double model_predictions[];
      ArrayResize(model_predictions, ArraySize(ensemble_handles));
      ...
      for(int i = 0; i < ArraySize(ensemble_handles); i++)
        {
         ...
         if(!RunPredictionForHandle(ensemble_handles[i], ensemble_model_names[i], features, model_prediction))
           {
            gLastEvaluationFailureStatus = "ensemble_inference_failed";
            return false;
           }

         model_predictions[i] = model_prediction;
         weighted_prediction_sum += model_prediction * weights[i];
         total_weight += weights[i];
         valid_models++;
        }

      if(valid_models <= 0 || total_weight <= 0.0)
        {
         gLastEvaluationFailureStatus = "ensemble_no_valid_models";
         return false;
        }

      prediction = weighted_prediction_sum / total_weight;
      ...
      for(int i = 0; i < ArraySize(model_predictions); i++)
        {
         int model_signal = 0;
         if(model_predictions[i] >= support_buy_threshold)
            model_signal = 1;
         else if(model_predictions[i] <= support_sell_threshold)
            model_signal = -1;

         if(model_signal == aggregate_signal)
           {
            support_count++;
            support_weight += weights[i];
           }
         else if(model_signal == -aggregate_signal)
           {
            opposing_count++;
            opposing_weight += weights[i];
           }
        }

      gLastEnsembleTrace = StringFormat(
         "Ensemble weighted vote | symbol=%s | pred=%.6f | support=%d(%.2f) | opposing=%d(%.2f) | xgboost=%.6f | lightgbm=%.6f | catboost=%.6f | sklearn_hgb=%.6f",
         _Symbol,
         prediction,
         support_count,
         support_weight,
      ...
      if(InpTraceEventWindows)
         DebugPrint(gLastEnsembleTrace);

      if(aggregate_signal == 0)
        {
         gLastEvaluationFailureStatus = "ensemble_neutral_vote";
         return false;
        }

      if(support_count < ResolveEnsembleMinimumSupport() || support_weight <= opposing_weight)
        {
         gLastEvaluationFailureStatus = "ensemble_consensus_blocked";
         return false;
        }
     }
   else
     {
      if(!RunPredictionForHandle(onnx_handle, ResolveModelString(), features, prediction))
         return false;
     }
...
string ResolveEventKey(const datetime event_time, const string event_name)
  {
   return TimeToString(event_time, TIME_DATE | TIME_MINUTES) + "|" + SanitizeCsvText(event_name);
  }
...
bool HasTradedEventKey(const string event_key)
  {
   for(int i = 0; i < ArraySize(gTradedEventKeys); i++)
     {
      if(gTradedEventKeys[i] == event_key)
         return true;
     }
   return false;
  }
...
void RememberTradedEventKey(const string event_key)
  {
   if(StringLen(event_key) == 0 || HasTradedEventKey(event_key))
      return;

   int count = ArraySize(gTradedEventKeys);
   ArrayResize(gTradedEventKeys, count + 1);
   gTradedEventKeys[count] = event_key;
  }
...
bool StringMatchesDelimitedKeyword(const string text, const string keyword_list)
  {
   ...
   string lower_keywords = keyword_list;
   StringToLower(lower_text);
   StringToLower(lower_keywords);

   int start = 0;
   while(start < StringLen(lower_keywords))
     {
      int sep = StringFind(lower_keywords, ";", start);
      string keyword = "";
      if(sep < 0)
        {
         keyword = StringSubstr(lower_keywords, start);
         start = StringLen(lower_keywords);
        }
      else
        {
         keyword = StringSubstr(lower_keywords, start, sep - start);
         start = sep + 1;
        }

      StringTrimLeft(keyword);
      StringTrimRight(keyword);
      if(StringLen(keyword) == 0)
         continue;
      if(StringFind(lower_text, keyword) >= 0)
         return true;
     }
...
string ResolveSymbolGeneralBlockedEventNames()
  {
   ...
   if(_Symbol == "EURUSD")
      blocked = "ECB Households Loans y/y;Retail Sales m/m";

   else if(_Symbol == "GBPUSD")
      blocked = "CFTC GBP Non-Commercial Net Pos;GDP q/q";

   else if(_Symbol == "USDJPY")
      blocked = "BoJ Trimmed Mean Core CPI y/y;CFTC JPY Non-Commercial Net Pos;Construction Orders y/y";
...
string ResolveSymbolModelBlockedEventNames()
  {
   if(_Symbol == "EURUSD")
     {
      if(InpModelType == MODEL_XGBOOST)
         return "Industrial Production m/m;Construction Output m/m;Current Account";
      if(InpModelType == MODEL_LIGHTGBM)
         return "S&P Global Construction PMI;Construction Output m/m;Industrial Production m/m;Consumer Price Expectations";
      if(InpModelType == MODEL_CATBOOST)
         return "S&P Global Construction PMI;Current Account;Construction Output m/m;CPI y/y;Consumer Price Expectations;Wage Costs y/y";
      if(InpModelType == MODEL_SKLEARN_HGB)
         return "Trade Balance;S&P Global Construction PMI;Current Account;Consumer Price Expectations;CPI y/y;Wage Costs y/y;Official Reserve Assets";
      return "S&P Global Construction PMI;Current Account;Industrial Production m/m;CPI y/y;Consumer Price Expectations;Construction Output m/m;Wage Costs y/y";
     }

   if(_Symbol == "AUDUSD")
     {
      if(InpModelType == MODEL_XGBOOST)
         return "RBA Index of Commodity Prices y/y;ANZ Job Advertisements m/m;CPI q/q;Trade Balance;Construction Work Done q/q;Export Price Index q/q;NAB Business Confidence;GDP q/q;Ai Group Industry Index;S&P Global Manufacturing PMI;S&P Global Services PMI";
      if(InpModelType == MODEL_LIGHTGBM)
         return "RBA Index of Commodity Prices y/y;NAB Business Confidence;MI Inflation Expectations;S&P Global Services PMI;CPI q/q;Construction Work Done q/q;PPI q/q;Building Approvals m/m;Ai Group Industry Index";
      if(InpModelType == MODEL_CATBOOST)
         return "RBA Index of Commodity Prices y/y;CPI q/q;Construction Work Done q/q;ANZ Job Advertisements m/m;Employment Change;NAB Business Confidence;Building Approvals m/m;Trade Balance;Export Price Index q/q;Wage Price Index q/q;S&P Global Manufacturing PMI;MI Inflation Expectations";
      if(InpModelType == MODEL_SKLEARN_HGB)
         return "RBA Index of Commodity Prices y;Trade Balance;Building Approvals m/m";
      return "RBA Index of Commodity Prices y/y;MI Inflation Expectations;Building Approvals m/m;Construction Work Done q/q;CPI q/q;Trade Balance;S&P Global Manufacturing PMI";
     }

   if(_Symbol == "GBPUSD")
     {
      if(InpModelType == MODEL_XGBOOST)
         return "GDP m/m;New Car Registrations m/m;HPI y/y;5-Year Treasury Gilt Auction;Manufacturing Production m/m;BoE Interest Rate Decision;Labour Productivity q/q;S&P Global/CIPS Manufacturing P";
      if(InpModelType == MODEL_LIGHTGBM)
         return "New Car Registrations m/m;BoE Consumer Credit m/m;Unemployment Rate;CPI y/y;5-Year Treasury Gilt Auction";
      if(InpModelType == MODEL_CATBOOST)
         return "New Car Registrations m/m;BoE Interest Rate Decision;CPI y/y;BoE Consumer Credit m/m;GDP m/m;HPI y/y;S&P Global/CIPS Construction P";
      if(InpModelType == MODEL_SKLEARN_HGB)
         return "RICS House Price Balance;CPI y/y;HPI y/y;S&P Global/CIPS Construction PMI;BoE Consumer Credit m/m;Public Sector Net Borrowing;30-Year Treasury Gilt Auction;NIESR GDP Estimate;CPIH m/m;Labour Productivity q/q";
      return "New Car Registrations m/m;BoE Consumer Credit m/m;BoE Interest Rate Decision;S&P Global/CIPS Manufacturing PMI;GDP m/m;5-Year Treasury Gilt Auction;Nationwide HPI m/m;CPI y/y;Labour Productivity q/q;HPI y/y";
     }

   if(_Symbol == "USDJPY")
     {
      if(InpModelType == MODEL_XGBOOST)
         return "Consumer Confidence Index;10-Year JGB Auction;Adjusted Trade Balance;Machine Tool Orders y/y;au Jibun Bank Manufacturing PMI;International Reserves;BoJ L Money Stock y/y;BoJ Interest Rate Decision;au Jibun Bank Services PMI;BoJ Bank Lending y/y;Core Machinery Orders m/m;Tokyo CPI excl. Food and Energy";
      if(InpModelType == MODEL_LIGHTGBM)
         return "10-Year JGB Auction;BSI Large Manufacturing;au Jibun Bank Manufacturing PMI;Retail Sales m/m;au Jibun Bank Services PMI;Tertiary Industry Activity Index m/m;BoJ Interest Rate Decision;Adjusted Trade Balance;BoJ L Money Stock y/y;CPI excl. Food and Energy y/y";
      if(InpModelType == MODEL_CATBOOST)
         return "Consumer Confidence Index;10-Year JGB Auction;au Jibun Bank Manufacturing PMI;au Jibun Bank Services PMI;Tertiary Industry Activity Index m/m;BoJ Interest Rate Decision;Core Machinery Orders m/m;Current Account n.s.a.;BoJ L Money Stock y/y;BSI Large Manufacturing";
      if(InpModelType == MODEL_SKLEARN_HGB)
         return "Consumer Confidence Index;Adjusted Trade Balance;BSI Large Manufacturing;Machine Tool Orders y/y;30-Year JGB Auction";
      return "au Jibun Bank Manufacturing PMI;10-Year JGB Auction;Consumer Confidence Index;Retail Sales m/m;Machine Tool Orders y/y;Adjusted Trade Balance;BSI Large Manufacturing;BoJ Interest Rate Decision;BoJ Bank Lending y/y;Core Machinery Orders m/m;Industrial Production m/m";
     }
...
string ResolveSymbolModelReversedEventNames()
  {
   if(_Symbol == "EURUSD")
     {
      if(InpModelType == MODEL_XGBOOST)
         return "S&P Global Construction PMI;Consumer Price Expectations";
      if(InpModelType == MODEL_LIGHTGBM)
         return "CPI y/y";
      if(InpModelType == MODEL_CATBOOST)
         return "";
      if(InpModelType == MODEL_SKLEARN_HGB)
         return "S&P Global Services PMI";
      return "";
     }
   ...
   if(_Symbol == "AUDUSD")
     {
      if(InpModelType == MODEL_XGBOOST)
         return "Building Approvals m/m;MI Inflation Expectations";
      if(InpModelType == MODEL_LIGHTGBM)
         return "S&P Global Manufacturing PMI;Employment Change;Trade Balance";
      if(InpModelType == MODEL_CATBOOST)
         return "CFTC AUD Non-Commercial Net Pos;Ai Group Industry Index";
      if(InpModelType == MODEL_SKLEARN_HGB)
         return "Employment Change";
      return "Employment Change;ANZ Job Advertisements m/m;CFTC AUD Non-Commercial Net Pos";
     }
   ...
   if(_Symbol == "GBPUSD")
     {
      ...
      if(InpModelType == MODEL_CATBOOST)
         return "";
      if(InpModelType == MODEL_SKLEARN_HGB)
         return "5-Year Treasury Gilt Auction;BoE Interest Rate Decision;GDP m/m";
      return "";
     }
   ...
   if(_Symbol == "USDJPY")
     {
      if(InpModelType == MODEL_XGBOOST)
         return "";
      if(InpModelType == MODEL_LIGHTGBM)
         return "Machine Tool Orders y/y";
      if(InpModelType == MODEL_CATBOOST)
         return "";
      if(InpModelType == MODEL_SKLEARN_HGB)
         return "";
...
bool EventTypeAllowed(const string event_name)
  {
   if(StringMatchesDelimitedKeyword(event_name, ResolveGlobalBlockedEventNames()))
      return false;
   if(StringMatchesDelimitedKeyword(event_name, ResolveSymbolGeneralBlockedEventNames()))
      return false;
   if(StringMatchesDelimitedKeyword(event_name, ResolveSymbolModelBlockedEventNames()))
      return false;
   return true;
  }
...
bool EventSignalShouldBeReversed(const string event_name)
  {
   if(StringMatchesDelimitedKeyword(event_name, ResolveGlobalReversedEventNames()))
      return true;
   if(StringMatchesDelimitedKeyword(event_name, ResolveSymbolGeneralReversedEventNames()))
      return true;
   return StringMatchesDelimitedKeyword(event_name, ResolveSymbolModelReversedEventNames());
  }

//+------------------------------------------------------------------+
bool PassTechnicalFilter(const datetime bar_time, const int signal)
  {
   if(!InpUseTechnicalFilter || signal == 0)
      return true;

   int shift = iBarShift(_Symbol, PERIOD_H1, bar_time, true);
   int shift_h4 = iBarShift(_Symbol, PERIOD_H4, bar_time, false);
   if(shift < 0 || shift_h4 < 0)
      return false;

   MqlRates rates[];
   ArraySetAsSeries(rates, true);
   if(CopyRates(_Symbol, PERIOD_H1, 0, shift + 2, rates) < (shift + 1))
      return false;
   ...
   if(signal > 0)
      return (close > ma_h4 && macd_main > macd_signal && rsi >= 50.0);
   return (close < ma_h4 && macd_main < macd_signal && rsi <= 50.0);
  }
...
bool IsInverseFundamentalEvent(const string event_name)
  {
   return StringMatchesDelimitedKeyword(event_name, gInverseEventKeywords);
  }
...
bool PassFundamentalDirectionFilter(const int signal,
                                    ...
                                    const double previous)
  {
   if(!InpUseFundamentalFilter || signal == 0)
      return true;
   ...
   bool has_forecast = (MathAbs(forecast) > 0.0000001);
   bool has_previous = (MathAbs(previous) > 0.0000001);
   if(!has_forecast && !has_previous)
      return false;
   ...
   int comparison_direction = 0;
   if(has_forecast && has_previous)
     {
      if(forecast_cmp != previous_cmp)
         return false;
      comparison_direction = forecast_cmp;
     }
   else if(has_forecast)
      comparison_direction = forecast_cmp;
   else
      comparison_direction = previous_cmp;

   if(comparison_direction == 0)
      return false;

   bool inverse_event = IsInverseFundamentalEvent(event_name);
   bool bullish_for_currency = false;
   if(!inverse_event)
      bullish_for_currency = (comparison_direction > 0);
   else
      bullish_for_currency = (comparison_direction < 0);

   int expected_signal = 0;
   if(event_country == base_code)
      expected_signal = (bullish_for_currency ? 1 : -1);
   else
      expected_signal = (bullish_for_currency ? -1 : 1);

   if(EventSignalShouldBeReversed(event_name))
...
string BuildPerRunDebugCsvFileName()
  {
   string base_name = gDefaultDebugCsvBaseName;
   if(StringLen(base_name) == 0)
      base_name = "ML_Regression_EA_Live_debug.csv";
   ...
   if(dot_pos >= 0)
     {
      stem = StringSubstr(base_name, 0, dot_pos);
      extension = StringSubstr(base_name, dot_pos);
     }

   MqlDateTime local_dt;
   TimeToStruct(TimeLocal(), local_dt);
   ...
   string run_suffix = StringFormat("%s_%s_%s_%s_%lu",
                                    _Symbol,
                                    ResolveTimeframeString(),
                                    ResolveModelString(),
                                    timestamp,
                                    (ulong)GetTickCount());

   return stem + "_" + run_suffix + extension;
  }
...
bool AppendDebugCsv(const datetime previous_bar_time,
                    ...
                    const string status)
  {
   ...
   if(StringLen(gDebugCsvFileName) == 0)
      gDebugCsvFileName = gDefaultDebugCsvBaseName;

   if(StringLen(gDebugCsvFileName) == 0)
      gDebugCsvFileName = "ML_Regression_EA_Live_debug.csv";
   ...
   FileWrite(handle,
             ...
             DoubleToString(effective_buy_threshold, 6),
             DoubleToString(effective_sell_threshold, 6),
...
bool ProcessRuntimeWindow(const datetime previous_bar_time, const datetime current_bar_time, const int current_spread)
  {
   ...
   double effective_buy_threshold = gActiveThresholdBuy;
   double effective_sell_threshold = gActiveThresholdSell;
   int importance = 0;
   string event_name = "";
   datetime selected_event_time = 0;
   string selected_event_country = "";
   double selected_event_actual = 0.0;
   double selected_event_forecast = 0.0;
   double selected_event_previous = 0.0;
   if(!EvaluateEvent(previous_bar_time,
                     prediction,
                     importance,
                     event_name,
                     selected_event_time,
                     ...
                     selected_event_previous))
     {
      string failed_status = gLastEvaluationFailureStatus;
      if(StringLen(failed_status) == 0)
         failed_status = "no_news_or_feature_data";
      AppendDebugCsv(previous_bar_time, current_bar_time, event_name, importance, prediction, effective_buy_threshold, effective_sell_threshold, 0, current_spread, false, failed_status);
      return false;
     }
   ...
   if(importance < min_importance)
     {
      AppendDebugCsv(previous_bar_time, current_bar_time, event_name, importance, prediction, effective_buy_threshold, effective_sell_threshold, 0, current_spread, false, "below_importance_filter");
      return false;
     }

   if(!EventTypeAllowed(event_name))
     {
      AppendDebugCsv(previous_bar_time, current_bar_time, event_name, importance, prediction, effective_buy_threshold, effective_sell_threshold, 0, current_spread, false, "event_type_filtered");
      return false;
     }

   int signal = 0;
   if(prediction >= effective_buy_threshold)
      signal = 1;
   else if(prediction <= effective_sell_threshold)
      signal = -1;
   if(signal == 0)
     {
      AppendDebugCsv(previous_bar_time, current_bar_time, event_name, importance, prediction, effective_buy_threshold, effective_sell_threshold, 0, current_spread, false, "below_threshold");
      return false;
     }
   ...
   if(EventSignalShouldBeReversed(event_name))
     {
      signal = -signal;
      DebugPrint(StringFormat(
         "Signal reversed by event rule | bar=%s | event=%s | prediction=%.6f | adjusted_signal=%d",
         TimeToString(previous_bar_time, TIME_DATE | TIME_MINUTES),
         event_name,
         prediction,
         signal
   ...
   if(!PassTechnicalFilter(previous_bar_time, signal))
     {
      AppendDebugCsv(previous_bar_time, current_bar_time, event_name, importance, prediction, effective_buy_threshold, effective_sell_threshold, signal, current_spread, false, "technical_filter_blocked");
      return false;
     }

   if(!PassFundamentalDirectionFilter(signal,
                                      event_name,
                                      selected_event_country,
                                      selected_event_actual,
                                      selected_event_forecast,
                                      selected_event_previous))
     {
      AppendDebugCsv(previous_bar_time, current_bar_time, event_name, importance, prediction, effective_buy_threshold, effective_sell_threshold, signal, current_spread, false, "fundamental_filter_blocked");
      return false;
     }

   string event_key = ResolveEventKey(selected_event_time, event_name);
   if(InpOneTradePerEvent && HasTradedEventKey(event_key))
     {
      AppendDebugCsv(previous_bar_time, current_bar_time, event_name, importance, prediction, effective_buy_threshold, effective_sell_threshold, signal, current_spread, false, "event_already_traded");
      return false;
     }
   ...
   if(InpMaxSpread > 0 && current_spread > InpMaxSpread)
     {
      ...
      AppendDebugCsv(previous_bar_time, current_bar_time, event_name, importance, prediction, effective_buy_threshold, effective_sell_threshold, signal, current_spread, false, "spread_blocked");
      return false;
     }
   ...
   if(executed && InpOneTradePerEvent)
      RememberTradedEventKey(event_key);
   AppendDebugCsv(previous_bar_time, current_bar_time, event_name, importance, prediction, effective_buy_threshold, effective_sell_threshold, signal, current_spread, executed, (executed ? "trade_executed" : "trade_failed_or_skipped"));
   return executed;
  }
...
int OnInit()
  {
   ...
   if(_Period != PERIOD_H1)
     {
      Print("ML_Regression_EA_Live supports H1 only. Current period: ", EnumToString(_Period));
      return INIT_FAILED;
     }

   trade.SetExpertMagicNumber(InpMagicNumber);

   gAssetFolder = InpAssetFolder;
   gNewsEventsFileName = InpNewsCsvFileName;
   ...
   if(InpDebugCsvPerRun)
      gDebugCsvFileName = BuildPerRunDebugCsvFileName();
   else
      gDebugCsvFileName = gDefaultDebugCsvBaseName;
   ArrayResize(gTradedEventKeys, 0);
   ...
   Print("ML_Regression_EA_Live initialized successfully. Symbol: ", _Symbol,
         " | Model: ", ResolveModelString(),
         " | AssetFolder: ", gAssetFolder,
         " | Scaler: ", gScalerFileName,
         " | News file: ", gNewsEventsFileName,
         " | Transactions CSV: ", gTransactionCsvFile,
