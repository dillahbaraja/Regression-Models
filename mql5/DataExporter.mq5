...
// --- Inputs ---
input int InpHistoryBars = 150000;    // Maximum historical bars to scan on H1
...
input datetime InpStartTime = D'2018.01.01 00:00';  // Export start time
input datetime InpEndTime = D'2023.12.31 23:59';   // Export end time
...
void ExportData()
  {
   ...
   for(int i = copied - 1; i >= 0; i--)
     {
      if(rates[i].time < InpStartTime || rates[i].time > InpEndTime)
         continue;
   ...
   Print("Data exported successfully to: ", gOutputFileName, " for symbol ", gTargetSymbol,
         ". Rows exported=", exportedRows,
         ", range=", TimeToString(InpStartTime, TIME_DATE|TIME_MINUTES), " to ",
         TimeToString(InpEndTime, TIME_DATE|TIME_MINUTES));
   ExpertRemove();
  }
